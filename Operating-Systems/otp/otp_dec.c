...
CS344
OSU ID: 933239746
otp_dec: This is the client process that accepts
a ciphertext file and key, and connects on the
provided portto otp_dec_d.
Then sends the file+key to otp_dec_d, accepts the plaintext
response, and writes plaintext to stdout.
**********************************************/
...
int main(int argc, char *argv[])
{
  ...
  int cipherText = open(argv[1], O_RDONLY);
  int cipherTextSize = lseek(cipherText, 0 , SEEK_END);
  ...
  //send the text and they key to the server for decryption
  ...
  if (charsRead < 0)
    fprintf(stderr, "otp_dec: Error reading encrypted message from socket");
...
void sendToServer(char* filename, char* filename2, int socketFD)
{
  ...
  //open ciphertext file and copy to buffer
  ...
  if(f != NULL)
  {
    ...
    if(ferror(f) != 0)
      fprintf(stderr, "otp_dec: Error reading %s", filename);
  ...
  if(f != NULL)
  {
    ...
    if(ferror(f) != 0)
      fprintf(stderr, "otp_dec: Error reading %s", filename2);
  ...
  //now send the actual ciphertext + * + key
  int charsWritten = 0;
  do
  {
...
int setUpConnection(char* arg3)
{
  ...
  if(connect(socketFD, (struct sockaddr*) &serverAddress, sizeof(serverAddress)) < 0)
    error("otp_dec: ERROR connecting", 0);
  ...
  if(strcmp(buffer, "+") == 0)
  {
      fprintf(stderr, "otp_dec: connection attempt with otp_enc_d on port %d was rejected.", portNum);
      exit(2);
  }
  else if(strcmp(buffer, "!") != 0)
  {
    fprintf(stderr, "otp_dec: connection attempt on port %d was rejected.", portNum);
    exit(2);
  }
  return socketFD;
}
