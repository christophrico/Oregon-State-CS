...
CS344
OSU ID: 933239746
otp_enc_d: This is the 'daemon' process that waits
for an incoming connection from otp_enc,
then encodes the plaintext message with the provided key
**********************************************/
...
/*********************************************
  ...
  Accepts connection from otp_enc process and authenticates
  Recv in the plaintext + key and separates them
  Encrypts plaintext by calling encryptText
  Sends ciphertext back to otp_enc
**********************************************/
void childProc(int connectFD)
{
  int i,
  ...
  //send the encrypted message back to otp_enc
  ...
  //now send the actual encrypted message back
  int charsWritten = 0;
  do
  {
...
/*********************************************
  ...
  Encrypts using a One-Time-Pad method
**********************************************/
void encryptText(char** plainText, char**key)
{
  int i,
