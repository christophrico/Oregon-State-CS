...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    Deque *stk = dequeNew();
    dequePushBack(stk, source);
    
    while(!found && !dequeIsEmpty(stk))
    {
      Vertex* current = dequeBack(stk);
      dequePopBack(stk);
      current->isVisited = 1;
      ...
      for (int i = 0; i < current->numNeighbors; i++)
      {
        if ((current->neighbors[i]->isVisited) == 0)
          dequePushBack(stk, current->neighbors[i]);
      }
    }
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    Deque *que = dequeNew();
    dequePushBack(que, source);
    
    while(!found && !dequeIsEmpty(que))
    {
      Vertex* current = dequeFront(que);
      dequePopFront(que);
      current->isVisited = 1;
      ...
      {
        if ((current->neighbors[i]->isVisited) == 0)
          dequePushBack(que, current->neighbors[i]);
      }
    }
